...
<%
	...
	try {
		lineSize = (Long) request.getAttribute("lineSize");
	} catch (Exception e) {
	}
	try {
...
<div align="center">
	<div id="pageCtl">
		<ul class="pagination">
			<li class="<%=currentPage == 1 ? "active" : ""%> div-float">
				<%
					if (currentPage == 1) {
				%> <span class="current">1</span> <%
 	} else {
 %> <a
				href="<%=url%>?cp=<%=1%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&${paramName}=${paramValue}">1</a>
			</li>
			<%
				}
			%>
			...
			%>
			<li class="<%=currentPage == x ? "active" : ""%> div-float"><a
				href="<%=url%>?cp=<%=x%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&${paramName}=${paramValue}"><%=x%></a></li>
			<%
				}
						if ((currentPage + seed * 2) < pageSize - 1) {
			%>
			<li class="disabled div-float"><span class="current">...</span></li>
			<%
				}
					} else { // 需要分两段显示了
			%>
			...
			<%
				if (pageSize != 1) {
			%>
			<li class="<%=currentPage == pageSize ? "active" : ""%> div-float">
				<%
			...
			%>
			<a
				href="<%=url%>?cp=<%=pageSize%>&ls=<%=lineSize%>&col=<%=column%>&kw=<%=keyWord%>&${paramName}=${paramValue}"><%=pageSize%></a>
			<%
				}
			%>
			</li>
