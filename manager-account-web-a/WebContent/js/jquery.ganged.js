;(function($){
	...
	function showProvince(obj, selectPId ,selectCId){
		if(currentOpts.selectAll){
	       defaultStr = '<li><a href="javascript:void(0);" val="-1">请选择</a></li>';
		}else{
			defaultStr = '';
		}
...
;(function($){
	...
	var selectbox = {
		...
		init: function(o) {
			var $o = $(o),
				_name = $o.attr('name'),
				_selectValue = $o.find('.opts >li > a.selected').attr('val')? $o.find('.opts >li > a.selected').attr('val'):-1,
				_selectHtml = $o.find('.opts >li > a.selected').html()? $o.find('.opts >li > a.selected').html():"请选择";
