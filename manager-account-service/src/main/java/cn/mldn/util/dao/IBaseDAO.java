...
public interface IBaseDAO<K, V> {
	 ...
	 * @return 由于返回多行的查询记录，所以每行记录都要求封装在VO类对象之中，如果没有返回任何查询结果，该集合长度为0(size()==0)，而不是null
	 */
	public List<V> findAllSplita(Integer currentPage,Integer lineSize) ;
	/**
	 * 数据模糊的分页查询操作
	 * @param map 当前所在页，此页数一定要大于0
	 * @return 由于返回多行的查询记录，所以每行记录都要求封装在VO类对象之中，如果没有返回任何查询结果，该集合长度为0(size()==0)，而不是null
	 */
	public List<V> findAllSplit(Map<String,Object> map) ;
	/**
	 * 是进行全部数据统计个数的查询，使用COUNT()函数
	 * @return 表中的全部记录数，如果表中没有记录，返回的就是0
	 */
	public Long getAllCounta() ;
	/**
